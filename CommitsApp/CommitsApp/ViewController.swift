...
class ViewController: UITableViewController {
    ...
    func configure(commit: Commit, usingJSON json: JSON) {
        ...
        if let authors = try? container.viewContext.fetch(authorRequest) { if authors.count > 0 { // we have this author already
                commitAuthor = authors[0]
            }
        }

        if commitAuthor == nil { // we didn't find a saved author - create a new one!
